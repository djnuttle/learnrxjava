...
public class ComposableListExercises<T> extends ArrayList<T> implements ComposableList<T> {
    ...
    */
    ...
    public static ComposableList<Integer> exercise11() {
        ...
        return movieLists
                .concatMap(movieList -> movieList.videos.map(video -> video.id));
    }
    ...
    public static ComposableList<JSON> exercise12() {
        ...
        return movieLists.concatMap(movieList -> {
            return movieList.videos.concatMap(video -> {
                ComposableList<BoxArt> boxarts = video.boxarts.filter(boxart -> boxart.height == 200 && boxart.width == 150);
    ...
    */
    ...
    In the previous example, theBinary function is invoked three times. (NO, 2 TIMES!)
    ...
    public ComposableList<T> reduce(BiFunction<T, T, T> combiner) {
        ...
        if (this.size() == 0) {
            return ComposableListExercises.of();
        } else {
    ...
    */
    ...
    public static ComposableList<String> exercise17() {
        ...
        return boxarts.reduce((b1, b2) -> {
            int size1 = b1.height * b1.width;
            int size2 = b2.height * b2.width;
            if (size1 > size2) {
                return b1;
            }
            return b2;
    ...
    public static ComposableList<Map<Integer, String>> exercise18() {
        ...
        return videos.
                ...
                for (Map.Entry<Integer, String> e : accumulatedMap.entrySet()) {
                    map.put(e.getKey(), e.getValue());
                }
                map.put(video.id, video.title);
                return map;

                 */
                            accumulatedMap.put(video.id, video.title);
                            return accumulatedMap;
                            //"can only change objects created inside the function"--this is not true, if mutating a map is considered changing it.
                            // In a multithreaded env, it might be a bad idea to mutate it (or it might not), but nothing prevents mutating it.
                            // ************ INSERT CODE HERE ************
        ...
        return movieLists.concatMap(movieList -> {
            return movieList.videos.concatMap(video -> {
                return video.boxarts
                        ...
                        .map(b -> json("id", video.id, "title", video.title, "boxart", b.url));
            });
        });
    }
    ...
    */
    public static ComposableList<JSON> exercise20() {
        ...
        for (int counter = 0; counter < Math.min(videos.size(), bookmarks.size()); counter++) {
            videoIdAndBookmarkIdPairs.add(json("videoId", videos.get(counter).id,
    ...
    */
    ...
    public static ComposableList<JSON> exercise22() {
        ...
        return ComposableListExercises.zip(videos, bookmarks, (video, bookmark) -> {
            return json("videoId", video.id, "bookmarkId", bookmark.id);
        });
    }
    ...
    public static ComposableList<JSON> exercise23() {
        ...
        return movieLists.concatMap(movieList -> {
           return movieList.videos.concatMap(video -> {
               ComposableList<BoxArt> boxarts = video.boxarts.reduce((b1, b2) -> {
                   if (b1.height * b1.width < b2.height * b2.width) {
                       return b1;
                   }
                   return b2;
               });
               ComposableList<InterestingMoment> moments = video.interestingMoments.filter(im -> {
                   return im.type.equals("Middle");
               });
               return boxarts.concatMap(ba -> {
                   return moments.map(m -> {
                      return json("id", video.id, "title", video.title, "time", m.time, "url", ba.url);
                   });
               });
           });
        });
